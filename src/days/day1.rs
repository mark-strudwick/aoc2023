...
pub fn part1() -> Result<String> {
    let file_lines: Vec<String> = fs::read_to_string("./inputs/day1.txt").expect("Could not read file 'day1'").split("\n").map(|s| s.to_string()).collect();

    let mut total = 0;
    for l in file_lines.iter() {
        // Assumes that there will always be a digit as we won't be left with 0... find a better
        // way
        let mut first_num = 0;
        let mut last_num = 0;
        for c in l.chars() {
...
pub fn part2() -> Result<String> {
    let mut file_lines: Vec<String> = fs::read_to_string("./inputs/day1.txt").expect("Could not read file 'day1'").split("\n").map(|s| s.to_string()).collect();
